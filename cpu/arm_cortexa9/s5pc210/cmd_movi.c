...
#ifdef CONFIG_SM5S4210
		/* image 5 should be logo */
		image[5].start_blk = image[4].start_blk + image[4].used_blk;
		image[5].used_blk = PART_SIZE_LOGO/MOVI_BLKSIZE;
		image[5].size = PART_SIZE_LOGO;
		image[5].attribute = 0x10;
		strcpy(image[5].description, "logo");
		dbg("logo: %d\n", image[5].start_blk);

		/* image 6 should be RFS */
		image[6].start_blk = image[5].start_blk + image[5].used_blk;
		image[6].used_blk = RAW_AREA_SIZE/MOVI_BLKSIZE - image[6].start_blk;
		image[6].size = image[6].used_blk * MOVI_BLKSIZE;
		image[6].attribute = 0x8;
		strcpy(image[6].description, "ramdisk");
		dbg("rfs: %d\n", image[6].start_blk);
