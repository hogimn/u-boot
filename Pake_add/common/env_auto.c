...
int saveenv_nand(void)
{
	...
	printf("Erasing Nand... (erasesize = %d)\n", erasesize);
	//nand_erase(&nand_info[0], CFG_ENV_OFFSET, CFG_ENV_SIZE);
	if (nand_erase(&nand_info[0], CFG_ENV_OFFSET, erasesize)) {
	...
	if (ret || total != CFG_ENV_SIZE) {
		printf("%s: Write Error(0x%x, 0x%x) !!!\n", __func__, ret, total);
		return 1;
	}
...
int saveenv(void)
{
    if((GPCDAT_REG & 0xA0) == 0xA0) {
        printf("Saving ENV to NAND...\n");
        // by crazyboy, saveenv_nand_adv();
        saveenv_nand();
    } else if((GPCDAT_REG & 0xA0) == 0x00) {
        printf("Saving ENV to SD Card...\n");
        saveenv_movinand();
    } else 
...
void env_relocate_spec_nand(void)
{
	...
	ret = nand_read(&nand_info[0], CFG_ENV_OFFSET, &total, (u_char*)env_ptr);
  	if (ret || (total != CFG_ENV_SIZE) || (crc32(0, env_ptr->data, ENV_SIZE) != env_ptr->crc)) {
		printf("read Main ENV failed !!!\n");
		// read 2nd env
		ret = nand_read(&nand_info[0], CFG_ENV_OFFSET + (512*1024), &total, (u_char*)env_ptr);
  		if (ret || total != CFG_ENV_SIZE)
			return use_default();
