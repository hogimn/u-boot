...
#ifdef USB_DEBUG
uint sb_debug_cnt = 0;
#define LOG(fmt,arg...) printf("[USBD-CTL-HS] %s: " fmt, __func__ ,##arg)
#else
#define LOG(arg...)
#endif
...
{
volatile	__u32 ep0sr;
volatile	__u16 read_cnt, i;
volatile	__u16 read_buf[4]={0x0000, };
volatile	__u8 set_addr;
	...
	if (s3c_ep0_state == EP0_STATE_IDLE) {
		...
		LOG("bmRequestType:%X, bRequest:%X\n", dev_req.bmRequestType, dev_req.bRequest);
		LOG("wValue:%X, wIndex:%X, wLength:%X\n", dev_req.wValue, dev_req.wIndex, dev_req.wLength);
		LOG("Read Buf [0]:%X, [1]:%X, [2]:%X, [3]:%X\n", read_buf[0], read_buf[1], read_buf[2], read_buf[3]);

		switch ((int)(dev_req.bRequest)) {
...
//#if defined(CONFIG_S3C2450) || defined(CONFIG_S3C2416)
  ...
  //      GPGPU_REG &= ~(0x3<<20); /*GPG10 vbus detect pull-up/down disable */
//#else
...
//	if (RSTSTAT_REG & 0x8) RSTCON_REG |= (1<<16);
	...
	mdelay(20); /* phy reset must be asserted for at 10us */
	...
	USB_PHYCTRL_REG = (0x0<<3)|(0x0<<2)|(0x0<<1)|(0x0<<0);
...
//	USB_CLKCON_REG = (0x1<<31)|(0x1<<2)|(0x0<<1)|(0x1<<0);
	 ...
	 * USB1.1 HOST disable,USB2.0 PHY test enable
	 */
	USB_CLKCON_REG = (0x1<<31)|(0x1<<2)|(0x0<<1)|(0x0<<0);

	mdelay(50);

	LOG("S3C2450 USB Controller Core Initialized\n");
